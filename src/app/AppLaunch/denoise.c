...
void static ff_memcpy(void* dest, const void* src, size_t size)
{
#ifdef __arm__
    if (size >= 64)
    {
        size_t i;
        size_t neon_size = size & ~0x3F;  // 按64字节对齐
        size_t leftover_size = size & 0x3F;  // 剩余的字节数

        // 使用NEON指令复制64字节的块
        for (i = 0; i < neon_size; i += 64)
        {
            uint8x16x4_t vsrc = vld4q_u8((const uint8_t*)src + i);
            vst4q_u8((uint8_t*)dst + i, vsrc);
        }
        // 复制剩余的字节
        memcpy((uint8_t*)dst + neon_size, (const uint8_t*)src + neon_size, leftover_size);
    }
    else
    {
    ...
    if (size >= 32)
    {
        const size_t simdSize = sizeof(__m256i);
        const size_t remainder = size % simdSize;
        const size_t simdIterations = std::min(size / simdSize, static_cast<size_t>(-1));

        const __m256i* srcPtr = static_cast<const __m256i*>(src);
        __m256i* destPtr = static_cast<__m256i*>(dest);
        for (size_t i = 0; i < simdIterations; ++i) {
            _mm256_storeu_si256(destPtr + i, _mm256_loadu_si256(srcPtr + i));
        }

        const uint8_t* srcRemainder = static_cast<const uint8_t*>(src) + simdIterations * simdSize;
        uint8_t* destRemainder = static_cast<uint8_t*>(dest) + simdIterations * simdSize;
        memcpy(destRemainder, srcRemainder, remainder);
    }
    else
    {
        memcpy(dest, src, size);
...
static void NoiseEstimation(NoiseSuppressionC* self, float* lmagn, float* noise)
{
    ...
    if (self->updates < END_STARTUP_LONG) {
        ...
        ff_memcpy(noise, self->quantile, self->magnLen * sizeof(*noise));
    }
    else {
        ff_memcpy(noise, self->quantile, self->magnLen * sizeof(*noise));
    }
...
static void ComputeSpectralFlatness(NoiseSuppressionC* self, const float* magnIn, const float* logmagnIn)
{
    size_t i;
...
static void ComputeSnr(const NoiseSuppressionC* self, const float* magn, const float* noise, float* snrLocPrior, float* logSnrLocPrior, float* snrLocPost)
{
    size_t i;
...
static void ComputeSpectralDifference(NoiseSuppressionC* self, const float* magnIn)
{
...
static void SpeechNoiseProb(NoiseSuppressionC* self, float* probSpeechFinal, const float* snrLocPrior, const float* logSnrLocPrior, const float* snrLocPost)
{
    size_t i;
    int sgnMap;
...
static void FeatureUpdate(NoiseSuppressionC* self, const float* magn, const float* lmagn, int updateParsFlag)
{
    // Compute spectral flatness on input spectrum.
...
static void UpdateNoiseEstimate(NoiseSuppressionC* self, const float* magn, float* noise)
{
    size_t i;
...
static void UpdateBuffer(const int16_t* frame, size_t frame_length, size_t buffer_length, float* buffer)
{
    ...
    ff_memcpy(buffer, buffer + frame_length, sizeof(*buffer) * (buffer_length - frame_length));
    if (frame) {
        int i = 0;
...
static void FFT(NoiseSuppressionC* self, float* time_data, size_t time_data_length, size_t magnitude_length, float* real,
    float* imag, float* magn, float* lmagn, int prev_calc, float* signalEnergy, float* sumMagn)
{
    size_t i;
...
// Output:
//   * |time_data| is the signal in the time domain.
static void IFFT(NoiseSuppressionC* self, const float* real, const float* imag, size_t magnitude_length, size_t time_data_length, float* time_data)
{
    size_t i;
...
void WebRtcNs_AnalyzeCore(NoiseSuppressionC* self, const int16_t* speechFrame) {
    ...
    ff_memcpy(self->noise, noise, sizeof(*noise) * self->magnLen);
...
void WebRtcNs_ProcessCore(NoiseSuppressionC* self,
    ...
    ff_memcpy(self->magnPrevProcess, magn, sizeof(*magn) * self->magnLen);
    ff_memcpy(self->noisePrev, self->noise, sizeof(self->noise[0]) * self->magnLen);
    // Back to time domain.
