...
class ex_counting_semaphore {
public:
	static_assert(_Least_max_value >= 0,
		"The least maximum value for a ex_counting_semaphore must be nonnegative (N4950 [thread.sema.cnt]/2).");

	_NODISCARD static constexpr ptrdiff_t(max)() noexcept {
		return _Least_max_value;
	}

	constexpr explicit ex_counting_semaphore(const ptrdiff_t _Desired) noexcept /* strengthened */
		: _Counter(_Desired) {
		_STL_VERIFY(_Desired >= 0 && _Desired <= _Least_max_value,
			"Precondition: desired >= 0, and desired <= max() (N4950 [thread.sema.cnt]/5)");
	}

	ex_counting_semaphore(const ex_counting_semaphore&) = delete;
	ex_counting_semaphore& operator=(const ex_counting_semaphore&) = delete;
	...
	_NODISCARD_TRY_CHANGE_STATE ptrdiff_t get_lock_count() noexcept {
...
_EXPORT_STD using ex_binary_semaphore = ex_counting_semaphore<1>;
