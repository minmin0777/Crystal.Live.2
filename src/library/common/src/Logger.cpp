...
// 自定义的文件打开处理器
void Logger::open_handler(sinks::text_file_backend::stream_type& file_stream)
{
    // 在文件打开时做一些自定义的处理,比如修改文件权限、重命名文件等
    // 这里我们只是简单地输出一条日志
    std::string strInitContent;
    strInitContent = std::format("Live.{0}----日志记录启动\n版本: {1}\n库路径: {2}", m_logInfo.Channel, m_logInfo.Version, m_logInfo.Location);
    // 转换为utf-8编码
    std::string Out = boost::locale::conv::to_utf<char>(strInitContent, "GBK");
    file_stream << OUTPUT_LINE;
    file_stream << Out << "\n";;
    file_stream << OUTPUT_LINE;
}

void Logger::close_handler(sinks::text_file_backend::stream_type& file_stream)
{
    // 在文件打开时做一些自定义的处理,比如修改文件权限、重命名文件等
    // 这里我们只是简单地输出一条日志
    std::string strInitContent;
    strInitContent = std::format("Live.{0}----日志记录结束\n版本: {1}\n库路径: {2}", m_logInfo.Channel, m_logInfo.Version, m_logInfo.Location);
    // 转换为utf-8编码
    std::string Out = boost::locale::conv::to_utf<char>(strInitContent, "GBK");
    file_stream << OUTPUT_LINE << "\n";
    file_stream << Out;
...
bool Logger::InitBoostlog(const LogInfo& logInfo)
{
    m_logInfo = logInfo;
    ...
    strLogFileName << "Live." << m_logInfo.Channel << "_%Y%m%d_%3N.log";
    ...
    m_pFileBackend->set_open_handler(&open_handler);
    m_pFileBackend->set_close_handler(&close_handler);
    // 确保在程序退出时刷新并关闭文件流
    m_pFileBackend->auto_flush(true);
    // // 设置一个全局文件打开处理器,这个处理器在文件被打开时会被调用
    // m_pFileBackendAll->set_open_handler(&open_handler);
    // m_pFileBackendAll->set_close_handler(&close_handler);
    ...
    // 设置日志级别过滤器,这意味着只有设定级别以上的日志消息才会被输出,默认是INFO及以上级别。
    // warning:可以通过System.dat配置文件设置日志级别,0-6分为7级,分别对应 TRACE:0,DEBUG:1,INFO:2,WARN:3,ERROR:4,FATAL:5,HEAD:6
...
{
    std::string strLogDir = GetWorkingDirectory(true, true);
    strLogDir += "log/";
    return strLogDir;
}

const std::string Logger::GetWorkingDirectory(bool bEndSlash, bool bUseLinuxSlash)
{
    return Common::Utility::GetWorkingDirectory(bEndSlash, bUseLinuxSlash);
}
